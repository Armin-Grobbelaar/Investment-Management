...
def mock_db(monkeypatch):
    ...
    monkeypatch.setattr("investment_database_functions.psycopg2.connect", lambda *a, **k: connection)
...
def test_investment_already_exists(mock_db, mock_foreign_currency):
    ...
    cursor.execute.assert_any_call(
        "SELECT id, institution_name, investment_name, unit_currency, investment_type FROM investments"
    )
...
def test_add_investment_with_forex(mock_engine, mock_ticker, mock_db, monkeypatch):
    ...
    monkeypatch.setattr("investment_database_functions.native_currency", "ZAR")
    monkeypatch.setattr(
        "investment_database_functions.foreign_currency",
    ...
    cursor.fetchone.side_effect = [[42], [99]]  # IDs returned for inserts
