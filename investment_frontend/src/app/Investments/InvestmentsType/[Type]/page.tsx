...
import { Container, IconButton, Paper, Grid, Typography, Box, Card, CardContent } from "@mui/material";
...
export default function FilteredInvestmentsPage() {
  ...
  const [OtherTableRows, setOtherTableRows] = useState<TableRowInterface[]>([]);
  ...
  useEffect(() => {
    async function fetchFilteredData() {
      try {
        setLoading(true);
        console.log(`🚀 Fetching filtered investment data for type: ${filterValue}`);

        // Call API with filter parameters
        const response = await axios.get(
          `/api/dashboard_charts/Investments?filter_type=investment_type&filter_value=${encodeURIComponent(filterValue)}`
        );

        const chartData = response.data;
        console.log('✅ Filtered data loaded successfully');

        // Set date range from included timeseries data
        ...
        // For filtered views, use institution breakdown
        ...
        console.log(`💰 Filtered portfolio shows ${chartData.summary_table?.length || 0} investments of type: ${filterValue}`);
              ...
              <Paper elevation={3} sx={{ p: 3 }}>
                <Suspense fallback={<div style={{ height: '300px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <LineGraph
                    ...
                    minDate={minDate}
                    maxDate={maxDate}
                    title={`Value Trends - ${filterValue}`}
                    xAxisLabel="Date"
            ...
            <Grid size={{ xs: 12 }}>
              <Paper elevation={3} sx={{ p: 3 }}>
                <Suspense fallback={<div style={{ height: '150px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <span style={{ marginLeft: '10px' }}>Loading Table...</span>
                </div>}>
            ...
            <Grid size={{ xs: 12, lg: 6 }}>
              <Paper elevation={3} sx={{ p: 3, height: '400px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Suspense fallback={<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
                  ...
                  <span style={{ marginTop: '10px' }}>Loading Chart...</span>
                </div>}>
