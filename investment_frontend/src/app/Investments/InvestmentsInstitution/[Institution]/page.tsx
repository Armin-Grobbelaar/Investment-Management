...
export default function FilteredInstitutionPage() {
  ...
  useEffect(() => {
    async function fetchFilteredData() {
      try {
        setLoading(true);
        console.log(`🚀 Fetching filtered investment data for institution: ${filterValue}`);

        const response = await axios.get(
          `/api/dashboard_charts/Investments?filter_type=institution_name&filter_value=${encodeURIComponent(filterValue)}`
        );
        ...
        setOtherPieValues(chartData.type_pie || null);
        setOtherTableRows(chartData.type_table || []);
            ...
            <Grid size={{ xs: 12, lg: 6 }}>
              <Paper elevation={3} sx={{ p: 3, height: '400px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Suspense fallback={<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
                  ...
                  <PieChart data={OtherPieValues} title="By Type" theme={darkMode ? 'dark' : 'light'} />
                </Suspense>
              </Paper>
            </Grid>
            <Grid size={{ xs: 12, lg: 6 }}>
              <Paper elevation={3} sx={{ p: 3 }}>
                <Suspense fallback={<div style={{ height: '150px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <StickyHeadTable
                    columns={[
                      { id: "investment_type", label: "Type", minWidth: 150 },
                      { id: "investment_value_in_native_currency", label: "Value in R", minWidth: 100 },
